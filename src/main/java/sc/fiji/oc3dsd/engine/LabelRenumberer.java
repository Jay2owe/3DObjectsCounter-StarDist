...
/**
 * Renumbers a label image so that surviving objects carry contiguous labels
 * {@code 1..N} in a deterministic order.
 * <p>
 * <strong>Why this exists.</strong> TrackMate's {@code LabelImgExporter} paints
 * {@code trackId + 1} into the label image, and track IDs are neither contiguous
 * nor stable: filtering objects out leaves holes, and the same object can carry
 * a different number between two runs of the same image with different
 * parameters. Every other 3D object counter numbers objects {@code 1..N}, users
 * join tables to maps on that number, and "object 6" has to mean something. So
 * the detector's numbering is treated as an internal detail and replaced, with
 * the original track ID preserved in its own column for anyone who needs to
 * trace a result back to a TrackMate session.
 * <p>
 * <strong>The order</strong> is first slice of appearance, then centroid Y, then
...
public final class LabelRenumberer {
    ...
    public static final class Result {
        ...
        /** Maps a pre-renumber label (TrackMate track ID + 1) to its new {@code 1..N} label. */
        public Map<Integer, Integer> oldToNew() {
            return Collections.unmodifiableMap(oldToNew);
        }

        /** Maps a new {@code 1..N} label back to the label the detector produced. */
    ...
    public static Result renumber(ImagePlus labelImage, Set<Integer> keep) {
        ...
        // Pass 2 — rewrite. Labels absent from the map become background.
        for (int slice = 1; slice <= nSlices; slice++) {
            ImageProcessor ip = stack.getProcessor(slice);
            if (ip == null) continue;
            int pixels = ip.getPixelCount();
            for (int i = 0; i < pixels; i++) {
                int label = labelOf(ip.getf(i));
                if (label <= 0) continue;
                Integer mapped = oldToNew.get(Integer.valueOf(label));
