...
/**
 * Proves that this plugin's reimplemented corrected surface agrees with mcib3d,
 * the library it claims to reproduce.
 *
 ...
 *
 * <p>Two shipped plugins therefore produce the same two columns by two different
 * code paths. The pom and the CHANGELOG both assert the definitions are the same;
 * until Stage 03 of the {@code oc3d-core} migration nothing checked it. An
 * undetected divergence here is the worst kind: no error, no warning, just two
 * tools in the same family reporting different shapes for the same object.
 *
 ...
 *
 * <p>mcib3d is a <b>test-scoped</b> dependency and is not shipped; see the
 * comment on it in {@code pom.xml}.
 */
public class CorrectedSurfaceAgainstMcib3dTest {
    ...
    @Test
    public void sphericityAndCompactnessMatchMcib3dAcrossShapes() {
        List<String> failures = new ArrayList<String>();
        List<String> observed = new ArrayList<String>();
        int liveComparisons = 0;
        ...
        for (Shape shape : shapes()) {
            ...
            Object3DInt object = new Object3DInt(ImageHandler.wrap(labels), 1);
            MeasureCompactness reference = new MeasureCompactness(object);
            double theirSphericity =
                    reference.getValueMeasurement(MeasureCompactness.SPHER_CORRECTED).doubleValue();
            double theirCompactness =
                    reference.getValueMeasurement(MeasureCompactness.COMP_CORRECTED).doubleValue();

            liveComparisons += check(failures, shape.name, "sphericity",
                    ours.sphericity(), theirSphericity);
            liveComparisons += check(failures, shape.name, "compactness",
                    ours.compactness(), theirCompactness);

            observed.add(shape.name + " sphericity=" + ours.sphericity()
                    + "/" + theirSphericity + " compactness=" + ours.compactness()
                    + "/" + theirCompactness);
        }

        // Without this, the test would pass if mcib3d returned NaN for everything
        // and check() kept taking its both-NaN early return - agreement on nothing.
        assertEquals("every shape must yield a finite value on both sides; anything else "
                        + "means the comparison did not actually run:\n  " + join(observed),
                2 * shapes().size(), liveComparisons);

        assertTrue("this plugin and mcib3d disagree on the corrected shape measures, so "
                        + "Morph_Sphericity and Morph_Compactness do NOT mean the same thing here as "
                        + "in 3D Objects Counter+:\n  " + join(failures),
                failures.isEmpty());

        System.out.println("corrected shape measures vs mcib3d " + MCIB3D_VERSION
                + " (this plugin/mcib3d)\n  " + join(observed));
    }
     ...
     */
    @Test
    public void anIsolatedVoxelHasNoCorrectedShapeInEitherImplementation() {
        ...
        Object3DInt object = new Object3DInt(ImageHandler.wrap(labels), 1);
        Double theirs = new MeasureCompactness(object)
                .getValueMeasurement(MeasureCompactness.SPHER_CORRECTED);
        assertFalse("mcib3d reported a finite sphericity for a lone voxel (" + theirs
                        + "); this plugin reports none, so the two disagree on the degenerate case",
                theirs != null && isUsable(theirs.doubleValue()));
    }
    ...
    /** @return 1 if this was a real comparison of two finite values, else 0. */
    private static int check(List<String> failures,
                             String shape,
                             String measure,
                             double ours,
                             double theirs) {
